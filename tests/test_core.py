...
# What a card that has none of them is actually offered. An optional core is
# not offered until it is installed: not having it is a choice, not a version
# that has fallen behind, so it is absent from outdated() and from a bare-card
# install. Once it is on the card it behaves like every other core.
OFFERED = tuple(c for c in RELEASED if not c.optional)
...
class Versions(unittest.TestCase):
    ...
    def test_an_absent_optional_core_is_not_outdated(self) -> None:
        ...
        rels = releases("1.4.0-cheats.9")
        out = [c.id for c in core.outdated(every(None), rels)]
        for c in core.CORES:
            if c.optional:
                self.assertNotIn(c.id, out)

    def test_an_installed_optional_core_updates_like_any_other(self) -> None:
        rels = releases("1.4.0-cheats.9")
        for c in core.CORES:
            if not c.optional:
                continue
            have = every(None) | {c.id: "0.0.1"}
            self.assertIn(c.id, [x.id for x in core.outdated(have, rels)])
...
class StatusLine(unittest.TestCase):
    ...
    def test_a_fifth_core_does_not_make_the_line_longer(self) -> None:
        """The dumper is a real core now, and it still does not lengthen it.

        Its version has to match its own release for this to be a fair
        comparison. Left behind, the line grows because it has something true
        to say, which is a different property and is tested elsewhere.
        """
        without = every("2.0") | {self.DUMPER.id: None}
        with_it = every("2.0") | {self.DUMPER.id: self.DUMPER_VERSION}
        current = releases("2.0")
        current[self.DUMPER.repo] = dict(current[self.DUMPER.repo],
                                         version=self.DUMPER_VERSION)
        for rels in (None, current):
            a, _ = core.describe(core.Survey("/card", without, []), rels)
            b, _ = core.describe(core.Survey("/card", with_it, []), rels)
...
class InstallEnd(Env):
    ...
    def test_a_bare_card_gets_every_core_that_has_a_release(self) -> None:
        ...
        # The optional core is not among them: a bare card is not missing it.
        self.assertEqual(core.installed(self.root),
...
class ZipShape(unittest.TestCase):
    """A release zip may only name paths the core it claims to be owns.

    Staying inside the card was the old rule and it is not enough: the card
    also carries ROMs, saves, cheat files and boot ROMs, and the README
    promises an install does not touch them. That promise is this class.
    """

    CORE = BY_ID["kroy.GBC"]

    def zip_with(self, *names: str) -> zipfile.ZipFile:
        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as z:
            z.writestr(f"Cores/{self.CORE.id}/core.json", "{}")
            for n in names:
                z.writestr(n, "x")
        buf.seek(0)
        return zipfile.ZipFile(buf)

    def test_its_own_directories_are_allowed(self):
        z = self.zip_with(f"Cores/{self.CORE.id}/bitstream.rbf_r",
                          "Platforms/gbc.json",
                          "Platforms/_images/gbc.bin",
                          f"Assets/{self.CORE.platform}/common/.gitkeep",
                          "instructions.txt")
        self.assertTrue(core._members(z, self.CORE))

    def test_another_platforms_assets_are_refused(self):
        with self.assertRaises(RuntimeError):
            core._members(self.zip_with("Assets/gba/common/gba_bios.bin"),
                          self.CORE)

    def test_its_own_asset_tree_is_allowed_in_the_archive(self):
        z = self.zip_with(f"Assets/{self.CORE.platform}/common/gbc_bios.bin")
        self.assertTrue(core._members(z, self.CORE))


    def test_saves_are_refused(self):
        with self.assertRaises(RuntimeError):
            core._members(self.zip_with("Saves/gbc/common/Game.sav"),
                          self.CORE)

    def test_another_core_is_refused(self):
        with self.assertRaises(RuntimeError):
            core._members(self.zip_with("Cores/someone.else/core.json"),
                          self.CORE)

    def test_a_nested_root_directory_is_refused(self):
        with self.assertRaises(RuntimeError):
            core._members(self.zip_with("Whatever/thing.bin"), self.CORE)

    def test_traversal_is_still_refused(self):
        for bad in ("../outside.bin", "/etc/passwd", "C:/windows/x.bin"):
            with self.assertRaises(RuntimeError):
                core._members(self.zip_with(bad), self.CORE)


class PlaceOverwrite(unittest.TestCase):
    """What _place is allowed to overwrite once the archive is accepted."""
    ...
    def put(self, base: str, rel: str, data: str) -> str:
    ...
    def test_a_boot_rom_already_on_the_card_is_not_overwritten(self):
        mine = self.put(self.card, "Assets/gbc/common/gbc_bios.bin", "MINE")
        self.put(self.staged, "Assets/gbc/common/gbc_bios.bin", "THEIRS")
        self.put(self.staged, f"Cores/{self.CORE.id}/core.json", "{}")
        core._place(self.staged, self.card, self.CORE)
        self.assertEqual(open(mine).read(), "MINE")

    def test_a_rom_already_on_the_card_is_not_overwritten(self):
        mine = self.put(self.card, "Assets/gbc/common/Game.gbc", "MINE")
        self.put(self.staged, "Assets/gbc/common/Game.gbc", "THEIRS")
        self.put(self.staged, f"Cores/{self.CORE.id}/core.json", "{}")
        core._place(self.staged, self.card, self.CORE)
        self.assertEqual(open(mine).read(), "MINE")

    def test_an_asset_the_card_lacks_is_written(self):
        self.put(self.staged, "Assets/gbc/common/.gitkeep", "")
        self.put(self.staged, f"Cores/{self.CORE.id}/core.json", "{}")
        core._place(self.staged, self.card, self.CORE)
        self.assertTrue(os.path.exists(
            os.path.join(self.card, "Assets/gbc/common/.gitkeep")))

    def test_platform_entries_are_replaced_because_the_core_owns_them(self):
        old = self.put(self.card, "Platforms/gbc.json", "OLD")
        self.put(self.staged, "Platforms/gbc.json", "NEW")
        self.put(self.staged, f"Cores/{self.CORE.id}/core.json", "{}")
        core._place(self.staged, self.card, self.CORE)
        self.assertEqual(open(old).read(), "NEW")
