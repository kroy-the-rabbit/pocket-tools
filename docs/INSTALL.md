...
```sh
chmod +x pocket-tools-*-linux-x86_64
./pocket-tools-*-linux-x86_64
```
...
run `pocket-tools.exe --check-db` from cmd: the exe has no console of its own
...
```sh
xattr -dr com.apple.quarantine "Pocket Tools.app"
open "Pocket Tools.app"
```
