...
cheatgui/run.sh --cart-dumps    # this run with the cartridge dumps switched on
```
...
Everything in this section appears when `kroy.CartTools` is installed on the
card and is gone when it is not. There is no setting: installing that core is
the act of asking for the feature, this app never installs it unasked, and a
card without it produces no dumps for any of this to read.
`core.dumper_installed()` is the whole gate, recomputed on every scan, install
and eject.

The core itself is always offered. It is in `core.CORES` like any other, marked
`optional`, so **Cores...** lists it and reports its version whether or not you
have it. Hiding it would be circular: this feature needs that core, and the
place anybody looks for a core is the core installer. What `optional` buys is
that an absent one is never counted as an update, so nothing ever ticks it for
you.

Gone rather than greyed: no **Cartridge dumps...** button on the card line, no
**Cartridge dumps** category in the systems pane, no **Copy to card**, and
nothing reads the library or loads a DAT. A disabled button promises that the
thing exists and is temporarily unavailable, which for somebody without the
dumper core is neither true nor useful.

The setting is yours and nothing else touches it. Installing `kroy.CartTools`
does not switch it on and removing it does not switch it off: what is on the
card says what the hardware can do, not what this window should show.
...
- the console the exe was launched from. Windows does not hand a windowed
  ...
  `pocket-tools.exe --list` from cmd prints into that cmd;
